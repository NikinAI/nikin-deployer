...
class NoDeltaService(deltaRepository: DeltaRepository) extends DeltaCalculatorService {
  private def generateDeletions(existingPipeline: InterpretedPipeline): List[Delete[_]] =
    existingPipeline
      ...
      .collect {
        ...
        case IngestService(name, address) => Delete(HttpService(name, address))
      }
  ...
  override def calculateDelta(updatedPipeline: InterpretedPipeline): Task[List[NodeUpdate[_]]] = {
    ...
    val deleteExisting = existingGraph.map(_.map(generateDeletions(_)).getOrElse(List.empty))

    val deltaUpdates =
      updatedPipeline
        .definitions
        .values
        .collect { case Lake(name, schema, ddl) => Create(LakeNode(name, ddl)) }
        .toList
    ...
    val recreate =
      updatedPipeline
        ...
        .collect {
          ...
          case IngestService(name, address) => Create(HttpService(name, address))
        }
        .toList
